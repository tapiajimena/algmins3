...
public class Punto {
	...
	public boolean equals(Object otroObjeto) {
		...
		if (otroObjeto != null && otroObjeto instanceof Punto) {
			...
			salida = (this.posicionX == otroPunto.posicionX)
					&& (this.posicionY == otroPunto.posicionY);
		}
		return salida;
	...
	public double distancia(Punto punto) {
		...
		if (distanciaX == 0)
			return Math.abs(distanciaY);
		if (distanciaY == 0)
			return Math.abs(distanciaX);
		return Math.pow(Math.pow(distanciaX, 2) + Math.pow(distanciaY, 2),
				1 / 2);
	}
