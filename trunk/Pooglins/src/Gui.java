...
public class Gui extends JFrame implements MouseListener {
	...
	char[][] matriz = {
		...
		{ 'R', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A',
			'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A',
			'A', 'A', 'A', 'A', 'A', 'A', 'T', 'T', 'T', 'T', 'A',
			'A', 'A', 'A', 'A', 'A', 'R' },
		{ 'R', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A',
			'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A',
			'A', 'A', 'A', 'A', 'A', 'A', 'T', 'T', 'F', 'F', 'F',
			'F', 'F', 'F', 'F', 'F', 'R' },
	...
	test.crearPoo("new", unPooglin.getPosicion().getX(), unPooglin
		.getPosicion().getY());
	...
	unLabel = new JLabel("", new ImageIcon("yeababyyea.jpg"), JLabel.LEFT);// GUI
									       // background
									       // for
									       // initial
									       // load
	...
	varx = contadorDeTiempo.getSeconds();// get the seconds allowed for the
					     // level;
	tiempoRestante = contadorDeTiempo.getMinutes();

	this.unPooglin = unP;

	jx = 0;// reset the variable used for keeping time to zero since its a
	       // new level
	// OJO
	tiempo = new Timer(1000, updateCursorAction);// create a timer to update
						     // the progress bar
	tiempo.start();// start the timer
	progPanel = new JPanel();// panel for progress bar
	progressBar = new JProgressBar(0, contadorDeTiempo.getMinutes() * 100);// minutes
									       // returns
									       // a
									       // single
									       // digit
	...
	public void actionPerformed(ActionEvent evt) {
	    ...
	    unPooglin.interactuar();
	    crearPoo("new", unPooglin.getPosicion().getX(), unPooglin
		    .getPosicion().getY());
	    //System.out.println(unPooglin.estaMuerto());
	}
    };
	...
	public void actionPerformed(ActionEvent e) // this inner class generates
						   // an exeption if the player
						   // takes to long to finish a
						   // level
	{
	    varx -= 1;
	    jx += 1;
